...
class Waterfall(object):
    ...
    def update(self, *args):
        ...
        for scan_num, start_ind in enumerate(range(0, NUM_SCANS_PER_SWEEP*NFFT, NFFT)):
            #sself.sdr.fc += self.sdr.rs*scan_num

            # estimate PSD for one scan
            samples = self.sdr.read_samples(NUM_SAMPLES_PER_SCAN)
            psd_scan, frequencies = psd(samples, NFFT=NFFT)
            print(frequencies[10:50])
            print(max(psd_scan))
            for idx, freq in enumerate(frequencies):
                if(freq > (tune_freq / 1e6) - 0.02 and freq < (tune_freq / 1e6) + 0.02 ):
                    print("{0}, {1}".format(freq, 10*log10(psd_scan[idx]))  )
                    #magnitudes.append("{0}, {1}".format(freq, 10*log10(psd_scan[idx]))  )    
